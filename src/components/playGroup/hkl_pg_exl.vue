<template>
  <table class="play-table" align="center">
    <tr v-if="!playgroup.alias">
      <th class="group-name" :colspan="setting.cols">
    ...
    <tr
      ...
      <td
        ...
        <el-col :span="15" class="number p-l">
          <span
            :class="[`play result-${gameCode} resultnum-${num}`]"
            v-for="(num,index) in zodiacMap&&zodiacMap[option.display_name]||tailMap[option.display_name]" :key="index">
          </span>
        </el-col>
      ...
      <td :colspan="setting.cols - optionChunk.length" v-if="optionChunk.length < setting.cols && optionChunkIndex === playgroup.plays.length - 1"></td>
    </tr>
  </table>
</template>
<script>
