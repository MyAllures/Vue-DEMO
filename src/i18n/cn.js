'use strict'

export default {
  navMenu: {
    home_page: '网站首页',
    game_center: '游戏大厅',
    guest: '还没有账号，免费注册',
    user: '已有账号，请登录',
    try_play: '免费试玩',
    user_login: '登录',
    forget_password: '忘记密码',
    login: '登录',
    logout: '退出登录',
    password: '密码',
    personal_account: '个人帐户',
    user_register: '注册',
    pop_title: '欢迎',
    bussiness_hours: '营业时间: 白天07:30 - 凌晨04:00/ 全年无休',
    qa: '常见问题',
    affliate: '加盟合作',
    promotion: '优惠活动',
    draw_history: '开奖走势',
    game_intro: '游戏介绍',
    result_period: '期',
    total: '总和',
    no_result: '尚无开奖结果',
    online_service: '在线客服',
    register_now: '立即注册'
  },
  agent: {
    register: '代理注册',
    username: '代理帐号',
    password: '設定密码',
    confirm_password: '确认密码',
    real_name: '真实姓名',
    realname_tip: '提醒：真实姓名必须与您用于提款银行账户名称一致',
    phone: '手机号码',
    email: '邮箱',
    captcha: '验证码',
    captcha_reset: '点击图片换一张'
  },
  promotions: {
    campaign: '活动',
    deposit_withdraw: '存提款',
    lottery: '类',
    others: '其他'
  },
  announcement: {
    title: '公告标题',
    speaker: '平台公告',
    content: '公告内容'
  },
  user: {
    username: '帐号',
    password: '密码',
    new_password: '新密码',
    prev_password: '当前密码',
    confirm_password: '确认密码',
    new_withdraw_password: '新取款密码',
    prev_withdraw_password: '当前取款密码',
    confirm_withdraw_password: '确认取款密码',
    balance: '余额',
    realname: '真实姓名',
    phone: '手机号码',
    email: '邮件信箱',
    qq: 'QQ',
    birthday: '生日',
    choose_date: '选择日期',
    wechat: '微信',
    withdraw_password: '取款密码',
    captcha: '验证码',
    level: '会员等级',
    money: '钱包',
    unsettled: '未结',
    unsettled_period: '未结期数',
    payment: ' ',
    payway: '支付方式',
    online_payment: '线上存款',
    remit: '转账 ',
    remit_bank: '入款银行',
    withdraw: '申请取款',
    betrecord: '下注纪录',
    personal_report: '个人报表',
    return_record: '反水纪录',
    betdate: '下注日期',
    bettime: '下注时间',
    my_account: '我的账号',
    primary_info: '基本信息',
    password_setting: '密码修改',
    bank_info: '银行信息',
    bank: '银行',
    city: '县市',
    province: '省份',
    amount: '入款金额',
    withdraw_amount: '取款金额',
    gender: '性别',
    male: '男',
    female: '女',
    transaction_time: '交易时间',
    transaction_amount: '交易金额',
    transaction_way: '交易方式',
    transaction_record: '交易纪录',
    distribute_time: '派发时间',
    distribute_game: '派发游戏',
    distribute_amount: '派发金额累计',
    distribute_status: '派发状态',
    payment_record: ' 纪录',
    discount_record: '优惠与红包',
    withdraw_record: '取款纪录',
    remit_record: '转帐 纪录',
    remit_account: '帐户',
    remit_address: '开户行网点',
    remit_despositor: '入款人',
    remit_time: '入款时间',
    remit_please_name: '请输入存款人姓名',
    remit_name: '存款人',
    issue_number: '期数',
    game_name: '游戏名称',
    play: '玩法',
    bet_count: '注数',
    bet_amount: '下注金额',
    status: '审核状态',
    success: '成功',
    failed: '失败',
    ongoing: '待处理',
    payment_pending: '处理中',
    cancelled: '已取消',
    no_draw: '官方未开',
    declined: '拒绝',
    finance: '财务纪录',
    message: '站内消息',
    change_password: '修改密码',
    announcement: '系统公告',
    sender: '寄件人',
    title: '标题',
    send_date: '寄件日期',
    content: '内容',
    weixin: '微信',
    alipay: '支付宝',
    bankcard: '银行卡',
    odd: '赔率',
    profit: '盈亏',
    visitor: '游客',
    min_amount: '最小金额',
    max_amount: '最大金额',
    unread_email: '未读信件',
    read_status: '状态',
    read: '已读',
    unread: '未读',
    agreement: '用户协议'
  },
  gameIntro: {
    gameinfo: '游戏资讯：',
    rule: '具体规则：',
    min_per_bet: '单注最低金额',
    max_per_bet: '单注最高金额',
    max_per_draw: '当期最高金额',
    return_rate: '反水',
    odds: '赔率'
  },
  history: {
    draw_period: '期数',
    draw_time: '开奖时间',
    draw_result: '开奖结果'
  },
  common: {
    all: '全部',
    please_select: '请选择',
    memo: '备注',
    select_date_time: '选择日期时间'
  },
  action: {
    submit: '提交',
    reset: '重置',
    modify: '修改',
    save: '保存',
    close: '关闭',
    setting: '设定'
  },
  validate: {
    username_validate: '请输入6~15位数字或字母',
    password_validate: '请输入6~15位数字或字母',
    withdraw_password_validate: '请输入6位纯数字',
    qq_validate: 'qq号码格式无效',
    realname_validate: '请输入中文姓名',
    province_validate: '请输入中文字符',
    phone_validate: '手机号码格式无效',
    wechat_validate: '微信格式无效',
    account_validate: '该帐号格式无效',
    password_again: '请再次输入密码',
    password_diff: '两次输入密码不一致',
    username_exist: '该帐号已存在',
    required: '该栏位必须输入',
    required_num: '该栏位必须输入数字',
    email_validate: 'email格式无效',
    not_set_bank_info: '尚未设定银行信息',
    max_amount_validate: '必须小于最大 金额',
    min_amount_validate: '必须大于最小 金额',
    max_withdraw_amount_validate: '必须小于最大取款金额',
    min_withdraw_amount_validate: '必须大于最小取款金额',
    date_range_validate: '起日不得晚于迄日',
    agreement_validate: '请阅读并同意用户协议'
  },
  message: {
    save_success: '保存成功',
    submit_success: '提交成功',
    error: '系统发生了错误, 请联系客服'
  }
}
