  ...
  <div class="form-wp">
    <el-form class="m-t-lg" method="post" target="_blank" :action="paymentUrl" :model="selectedPayment" ref="payment" status-icon :rules="rule" label-width="90px">
      <div>
        ...
        <el-form-item v-if="selectedPayment.client_description" :label="$t('common.memo')" prop="client_description">
          {{selectedPayment.client_description}}
        </el-form-item>
      </div>
    ...
    <el-dialog
      ...
      <slot name="footer">
        <el-row class="m-t-lg">
          ...
          <el-col :span="12" class="text-center">
            <span v-if="$store.state.systemConfig.serviceAction" @click="$store.state.systemConfig.serviceAction()">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rule: {
        amount: [
          { required: true, type: 'number', validator: limitPass, trigger: 'change' }
        ]
      },
      bank: '',
      activeType: '',
    ...
    fetchPaymentType().then(datas => {
      if (datas.length === 0) {
        ...
        datas.forEach((data, index) => {
          ...
          if (payment) {
            payment['client_description'] = data.client_description
  ...
  methods: {
    submit (e) {
      this.$refs['payment'].validate((valid) => {
        if (valid) {
          this.sendGaEvent({action: ' ', category: '在線支付', label: this.payeeDisplayName})
    ...
    select (type) {
      ...
      if (paymentType) {
        ...
        this.selectedPayment.client_description = paymentType.client_description
