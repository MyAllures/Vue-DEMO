<template>
  <div>
    ...
    <table class="play-table">
          ...
          <div>
            ...
            <el-popover
              ...
              <ul class="combinations">
                <li v-for="(combination, index) in combinations" :key="index">[{{combination.join(', ')}}]</li>
              </ul>
            </el-popover>
      ...
      <template v-for="(views, viewsIndex) in showOptionGroupViews">
        <tr :key="'views' + viewsIndex">
          <td :colspan="customPlayGroup.cols" align="center">
            <div>{{`第${viewsIndex+1}球`}}</div>
          </td>
        </tr>
        <tr v-for="(row, index) in views" :key="index">
          <td
